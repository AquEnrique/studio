...
import { DialogHeaderDirective, DialogTitleDirective } from '../../../ui/dialog/dialog-parts';
...
/**
 * Near-fullscreen on-screen keypad for entering a custom amount and applying it
 * to a player's life points as either a gain (+) or a loss (-), without relying
 * on the device's native keyboard. Closes itself with the delta to apply, or
 * `undefined` if dismissed without applying anything.
 */
@Component({
  selector: 'app-lp-keypad-dialog',
  standalone: true,
  imports: [LucideIcons, DialogHeaderDirective, DialogTitleDirective],
